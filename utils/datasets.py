...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    with torch_distributed_zero_first(rank):  # 多进程数据同步, 主进程处理数据, 其他进程读cache
        dataset = LoadImagesAndLabels(path, imgsz, batch_size,  # 构建dataset
    ...
    dataloader = loader(dataset,
                        ...
                        num_workers=1,#cjk:原版是num_workers=nw
                        sampler=sampler,
                        pin_memory=True,
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        # Padded resize
        #print("""原始图片cv2.imread**********************************************************************************************************************************""")
        #print(img0)
        img = letterbox(img0, self.img_size, stride=self.stride)[0]#填充resize
        #print(img)
        #print("""img=letterbox**********************************************************************************************************************************""")
        #print(img)
        ##原始的img0的像素排列为480行,854列
        ##填充后的img的像素排列为576行,1024列
        #print(len(img0),len(img0[0]),len(img0[0][0]))#-> 480 854 3
        #print(len(img),len(img[0]),len(img[0][0]))#-> 576 1024 3
        # Convert
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416 
        #二维矩阵的transpose相当于转置,三维的就是调换xyz轴,见https://www.cnblogs.com/caizhou520/p/11227986.html
        #print("""img[:, :, ::-1].transpose(2, 0, 1)**********************************************************************************************************************************""")
        ##print(len(img),len(img[0]),len(img[0][0]))#-> 3 576 1024
        #print(img)
        
        img = np.ascontiguousarray(img)#将一个内存不连续存储的数组转换为内存连续存储的数组，使得运行速度更快
        #print("""img=np.ascontiguousarray(img)**********************************************************************************************************************************""")
        #print(img)
