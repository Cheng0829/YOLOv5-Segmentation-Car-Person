...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:  # 有预训练参数
        ...
        exclude = ['anchor'] if (opt.cfg or hyp.get(
            'anchors')) and not opt.resume else []  # exclude keys 初始化时候不使用的参数(非resume且有配置时按cfg和model初始化来指定anchor, 否则延用pretrained weights的anchor)
        state_dict = ckpt[
            'model'].float().state_dict()  # to FP32 ckpt里model键对应的值才是模型,训练结束后保存的是float16(中间保存的float32),模型的state_dict是参数,包括可训练参数和register_buffer保存的buffer parameter(Detect的anchor和gird等)
        state_dict = intersect_dicts(state_dict, model.state_dict(),
                                     exclude=exclude)  # intersect 赋值参数,预训练的参数赋值到新建的模型,exclude除外(即anchor使用cfg的而不是预训练)
    ...
    # Optimizer
    nbs = 8  # cjk:原版是64  # nominal batch size
    ...
    imgsz, imgsz_test = [check_img_size(x, gs) for x in
                         opt.img_size]  # verify imgsz are gs-multiples 检查图片尺寸是否合法,不合法就自动替换
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:  # 常规, 非resume
            ...
            if not opt.noautoanchor:  # 用train dataset自动聚类选取最好anchor
                check_anchors(dataset, model=model, thr=hyp['anchor_t'],
                              imgsz=imgsz)  # anchor_t是最大放大倍数,yolov5公式不同于v3v4, 见核心Model推理时anchor偏移放缩公式和issue
        ...
        seg_valloader = SegmentationDataset.get_citys_loader(root=segval_path, batch_size=4,
                                                             ...
                                                             base_size=1024,
                                                             # 对cityscapes, 原图resize到(1024, 512)输入后双线性插值到原图尺寸计算精度
                                                             # crop_size=640,  # testval 时候cropsize不起作用
                                                             workers=4,
                                                             pin=True)  # 验证batch_size和workers得配合, 都太大会导致子进程死亡, 单进程龟速加载数据
    ...
    seg_trainloader = SegmentationDataset.get_citys_loader(root=segtrain_path,
                                                           ...
                                                           base_size=1024, crop_size=(imgsz, imgsz // 2),
                                                           # cropsize长边和检测同，短边砍半(针对cityscapes,bdd也行)
    ...
    nw = max(round(hyp['warmup_epochs'] * nb),
             800)  # number of warmup iterations, max(3 epochs, 1k iterations) 最少warmup三轮或500batch(原版1000,800就够了)
    # nw = min(nw, (epochs - start_epoch) / 2 * nb)  # limit warmup to < 1/2 of training
    maps = np.zeros(nc)  # mAP per class
    results = (0, 0, 0, 0, 0, 0, 0)  # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls)
    scheduler.last_epoch = start_epoch - 1  # do not move 配置lr_scheduler起始位置
    scaler = amp.GradScaler(
        enabled=cuda)  # 说明不是float16训练,而是16和32混合精度训练. 训练前初始化loss scaler 用于float16放大梯度后backward, optimizer.step之前自动转float32再缩回来
    ...
    compute_seg_loss = SegmentationLosses(aux=False, ignore_index=-1,
                                          weight=citys_class_weight).cuda()  # SegFocalLoss(ignore_index=-1, gamma=1, reduction="mean").cuda()
    # BiSe用这个　两个aux
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for det_batch, seg_batch in zip(pbar,
                                        segpbar):  # batch -------------------------------------------------------------
            i, (imgs, targets, paths, _) = det_batch  # 检测
            _, (segimgs, segtargets) = seg_batch  # 分割
            if len(imgs) == 1 or len(
                    segimgs) == 1:  # 手动droplast,SE或者gloablpool后的bn不支持单个样本，检测loader调用地方太多不好droplast，这里手动
                continue
            # warmup等参数变化以检测为准
            ...
            if ni <= nw:
                ...
                accumulate = max(1, np.interp(ni, xi, [1, math.floor(
                    nbs / total_batch_size)]).round())  # 梯度积累 线性插值xi=[0, 1000], yi=[1, 64/batchsize], 插入点x=ni, 之后取整, 最小限1. warmup时accumulate会逐渐从1按整数增大到目标, warmup结束后稳定在目标值 round(nbs/accumulate), 例如batchsize32实际上两batch才更新一次,等效于64
            ...
            if plots and ni >= 3:
                ...
                imgs = imgs.to(torch.device('cpu'),
                               non_blocking=True)  # 释放 segimgs输入后就没被调用会被pytorch自动回收不用手动释放(img后续有被调用要手动释放)
            ...
            with amp.autocast(enabled=cuda):  # 混合精度训练中用来代替autograd
                ...
                # Base,PSP和Lab用这个,无aux
                segloss = compute_seg_loss(pred[1], segtargets.to(
                    device)) * batch_size  # 分割loss CE是平均loss, 配合检测做梯度积累, 因此乘以batchsize(注意有梯度积累其真实batchsize约是nbs=64)
                # Bise用这个,两个aux   
        ...
        if rank in [-1, 0]:
            ...
            fi = fitness2(np.array(results).reshape(1, -1),
                          mIoU)  # weighted combination of [P, R, mAP@.5, mAP@.5-.95] 按0.1*AP.5+0.9*AP.5:.95指标衡量模型
...
if __name__ == '__main__':
    ...
    opt.global_rank = int(
        os.environ['RANK']) if 'RANK' in os.environ else -1  # global_rank是所有进程可用的GPU号, local_rank是当前进程对应GPU号
