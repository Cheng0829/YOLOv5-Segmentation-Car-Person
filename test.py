...
def segtest(weights, root="data/citys", batch_size=8, half_precision=True, n_segcls=19,
            base_size=2048):  # 会使用原始尺寸测, 未考虑尺寸对不齐, 图片尺寸应为32倍数
    # cjk:batch_size原版为16
    ...
    testvalloader = SegmentationDataset.get_citys_loader(root, batch_size=batch_size, split="val", mode="testval",
                                                         workers=1, base_size=base_size)  # cjk
...
if __name__ == '__main__':
    ...
    # 原版parser.add_argument('--batch-size', type=int, default=32, help='size of each image batch')
    parser.add_argument('--batch-size', type=int, default=8, help='size of each image batch')  # cjk
    parser.add_argument('--img-size', type=int, default=640, help='inference size (pixels)')
    parser.add_argument('--base-size', type=int, default=2048,
                        help='long side of segtest image you want to input network')
    ...
    if opt.task in ('train', 'val', 'test'):  # run normallyModel Summary:
        test(opt.data,
             opt.weights,
             opt.batch_size,
             opt.img_size,
    ...
    segtest(root=opt.segdata, weights=opt.weights, batch_size=int(opt.batch_size / 8), n_segcls=19,
            base_size=opt.base_size)  # 19 for cityscapes
